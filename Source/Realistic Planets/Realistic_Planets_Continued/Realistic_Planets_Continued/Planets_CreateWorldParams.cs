...
namespace Planets_Code
{
	public class Planets_CreateWorldParams : Page
	{
		...
		public override void DoWindowContents(Rect rect)
		{
            ...
            bool flag = true;
            foreach (FactionDef item2 in configurableFactions)
            {
                if (flag && factionCounts[item2] != item2.startingCountAtWorldCreation)
                {
                    flag = false;
                }
            }
            WorldFactionsUIUtility.DoWindowContents(rect8, ref factionCounts, flag, out bool resetFactions);
            if (resetFactions)
            {
                ResetFactionCounts();
        ...
        {
            factionCounts = new Dictionary<FactionDef, int>();
